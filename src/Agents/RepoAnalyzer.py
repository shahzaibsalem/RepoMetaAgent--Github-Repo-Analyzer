...
if __name__ == "__main__":
    repo_url = "https://github.com/roshan9419/LearnEd_E-learning_Website"
    
    # 1. Build and compile the graph
    app = build_analyzer_graph()
    
    # 2. Define the initial state (the only required input)
    initial_state = {"repo_url": repo_url}

    print(f"\n🚀 Starting LangGraph analysis for: {repo_url}")
    
    # 3. Run the graph
    # NOTE: The LangGraph is run with stream=True for real-time progress, but we
    # collect the final state.
    final_state = None
    for s in app.stream(initial_state):
        # s is a dictionary where the key is the node that just executed
        node_name = list(s.keys())[0]
        print(f"✅ Node executed: {node_name}")
        final_state = s

    # The final output is stored in the last element of the final state
    if final_state:
        final_result = final_state.get('formatter', {}).get('final_output', {})
